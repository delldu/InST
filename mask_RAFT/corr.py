...
class CorrBlock:
    ...
    # def corr2(self, fmap1, fmap2,q,k):
    #     num_heads = 2
    #     B,C,H,W = fmap1.shape
    #     N = H * W
    #     head_dim = C // num_heads
    #     scale = head_dim ** -0.5
    #     fmap1 = fmap1.reshape(B,C,N).permute(0,2,1)
    #     fmap2 = fmap2.reshape(B,C,N).permute(0,2,1)
    #     q_1 = q(fmap1).reshape(B, N, num_heads, C // num_heads).permute(0, 2, 1, 3)
    #     k_2 = k(fmap2).reshape(B, N, num_heads, C // num_heads).permute(0, 2, 1, 3)
    #     attn = (q_1 @ k_2.transpose(-2, -1)) * scale    # [B,num_head,N,N]
    #     corr = attn.view(B,num_heads,H,W,1,H,W)
    #     return corr / torch.sqrt(torch.tensor(head_dim).float())
