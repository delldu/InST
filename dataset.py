        ...
        if pair_txt != None:
            ...
            self.transform2 = transforms.Compose(
                [
                    ...
                    transforms.Pad((256 - 128) // 2, fill=(255, 255, 255)),
                    transform.transforms[1],
                ]
            )
            self.paths = sorted(
                glob.glob(os.path.join(data_root, "*.jpg")) + glob.glob(os.path.join(data_root, "*.png"))
            )
...
class FlatFolderDataset(Dataset):
    def __init__(self, data1_root, data2_root=None, transform=None):
        ...
        self.paths_1 = sorted(
            glob.glob(os.path.join(data1_root, "*.jpg")) + glob.glob(os.path.join(data1_root, "*.png"))
        )
        if self.data2_root != None:
            self.paths_2 = sorted(
                glob.glob(os.path.join(data2_root, "*.jpg")) + glob.glob(os.path.join(data2_root, "*.png"))
            )
